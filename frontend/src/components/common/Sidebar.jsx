...
import { Calendar, Users, UserCog, LogOut, BarChart2, Sun, Moon, Settings, MessageSquare, FlaskConical } from 'lucide-react'
  ...
  return (
    <>
      <div className="pt-2 pb-1 px-3">
        <span className="text-xs font-semibold text-orange-400 uppercase tracking-wide">⚡ スーパー管理者</span>
      </div>

      {/* テストモードトグル */}
      <div className="mx-3 my-1 rounded-lg border border-orange-200 bg-orange-50 px-3 py-2">
      ...
      {/* LINEデバッグ */}
      {[
        { path: '/admin/line-debug',    label: 'LINEデバッグ',     icon: MessageSquare },
        { path: '/admin/test-patients', label: 'テスト患者管理',   icon: Users },
  ...
  return (
    <aside className="w-56 bg-white border-r border-gray-200 flex flex-col overflow-y-auto">
      ...
      <nav className="flex-1 px-3 space-y-1">
        ...
        {/* 管理者専用メニュー（admin と superadmin 両方） */}
        {(localStorage.getItem('admin_role') === 'admin' || localStorage.getItem('admin_role') === 'superadmin') && (
          <>
            <div className="pt-2 pb-1 px-3">
            ...
            {[
              ...
              { path: '/admin/blocks',   label: '予約制限管理',   icon: Calendar },
            ].map(({ path, label, icon: Icon }) => {
              const active = location.pathname === path
              return (
                <Link
        ...
        {/* スーパー管理者専用メニュー */}
        {localStorage.getItem('admin_role') === 'superadmin' && (
          <SuperAdminSection />
        )}
      </nav>
