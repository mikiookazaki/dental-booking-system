...
import { Calendar, Users, UserCog, LogOut, BarChart2, Sun, Moon, Settings } from 'lucide-react'
...
const adminNavItems = [
  { path: '/admin/dashboard', label: 'ダッシュボード',   icon: BarChart2, feature: null       },
  { path: '/admin/settings',  label: 'システム設定',     icon: Settings,  feature: null       },
  // 以下、未実装ページは feature を設定しておくと将来ガードできる
  // { path: '/reminders',    label: 'リマインダー',     icon: Bell,      feature: 'standard' },
  // { path: '/segments',     label: 'セグメント配信',   icon: Tag,       feature: 'standard' },
  // { path: '/ai/faq',       label: 'AI FAQ',          icon: Bot,       feature: 'pro'      },
]
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    if (!token) return
    fetch(`${API}/api/licenses/default`, {
    ...
    <aside className="w-56 bg-white border-r border-gray-200 flex flex-col overflow-y-auto">
      ...
      <div className="px-3 pt-2 flex-shrink-0">
        <div style={{
          ...
          background: currentPlan === 'pro' ? '#FAEEDA' : currentPlan === 'standard' ? '#EEEDFE' : '#E1F5EE',
          fontSize: 11,
        }}>
          <span style={{ color: currentPlan === 'pro' ? '#854F0B' : currentPlan === 'standard' ? '#534AB7' : '#0F6E56', fontWeight: 600 }}>
            {PLAN_LABELS[currentPlan]}プラン
          </span>
          <Link to="/admin/settings" style={{ fontSize: 10, color: '#6b7280', textDecoration: 'none' }}>
            変更 →
          </Link>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 space-y-1">
        ...
        {localStorage.getItem('admin_role') === 'admin' && (
          <>
            ...
            {adminNavItems.map(({ path, label, icon: Icon, feature }) => {
              const active = location.pathname === path
              const locked = feature && !canUse(currentPlan, feature)
              return (
                <Link
